...
def setup_logger():
    """ Set up logging
    """
    logfilename = "process-ia-nyt-jsonp-topnews.log"
...
def parse_nyt_jsonp_topnews(fn, year):
    results = []
    html = open_html(fn)
    m = re.match(r'.*?\((\{.*\})\).*', str(html), flags=re.S)
    if m:
        js = m.group(1)
    # FIXME: some escape couldn't parse by json
    js = js.replace("\\'", "'")
    js = js.replace('\\"', '\"')
    js = js.replace('\\/', '/')
    ...
    j = json.loads(js)
    results = []
    for t in j['top_pages']:
        results.append((t['headline'], t['url']))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse NYT TopNews JSONP')
        ...
        for fn in (glob(args.directory + '/*.html') +
            ...
            if year in [2012, 2016]:
                if month in range(7, 13):
                    #print(src)
                    if src == 'nyt':
                        results = parse_nyt_jsonp_topnews(fn, year)
                    else:
                        print("Not implement parser for this news source: '%s'"
                              % src)
                        continue
                    new_results = []
